...
((exports) => {
  ...
  exports.runConvertTest = () => {
    ...
    const main = async () => {
      ...
      if (process.platform === 'win32' && PDFNet.Convert.printerIsInstalled()) {
        try {
    ...
    const convertToPdfFromFile = async () => {
      if (process.platform === 'win32') {
        if (PDFNet.Convert.printerIsInstalled("PDFTron PDFNet")) {
          PDFNet.Convert.printerSetPrinterName("PDFTron PDFNet");
        }
        else if (!PDFNet.Convert.printerIsInstalled()) {
          try {
      ...
      for (const testfile of testfiles) {
        ...
        try {
          ...
          pdfdoc.save(outputFile, PDFNet.SDFDoc.SaveOptions.e_linearized);
    ...
    const convertSpecificFormats = async () => {
      try {
        ...
        pdfdoc.save(outputPath + "xps2pdf v2.pdf", PDFNet.SDFDoc.SaveOptions.e_linearized);
      ...
      if (process.platform === 'win32') {
        try {
          ...
          pdfdoc.save(outputPath + "emf2pdf v2.pdf", PDFNet.SDFDoc.SaveOptions.e_linearized);
      ...
      try {
        ...
        pdfdoc.save(outputPath + "simple-text.pdf", PDFNet.SDFDoc.SaveOptions.e_remove_unused);
